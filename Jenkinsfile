            ...
            steps {
                ...
                withCredentials([usernamePassword(credentialsId: 'Web_Login', usernameVariable: 'USERNAME', passwordVariable: 'USERPASS')]) {
                    script {
                        ...
                        sh "sshpass -p '$USERPASS' -v ssh -o StrictHostKeyChecking=no $USERNAME@$prod_ip \"docker run --restart always --name train-schedule -p 9090:8080 -d ganman/train-schedule:${env.BUILD_NUMBER}\""
                    }
                }
            }
        }
